...
import com.sun.redislock.RedisLock;
...
public class ProductController {
	...
	private String queryMap(String productId) {// 模拟查询数据库
		return "国庆活动，皮蛋特教，限量" + products.get(productId) + "份,还剩:" + stock.get(productId) + "份,该商品成功下单用户数:"
				+ orders.size() + "人";
	}

	/**
	 * 基于Redis的分布式锁 http://redis.cn/commands/setnx.html
	 * http://redis.cn/commands/getset.html SETNX命令
	 * 将key设置值为value，如果key不存在，这种情况下等同SET命令。 当key存在时，什么也不做 GETSET命令
	 * 先查询出原来的值，值不存在就返回nil。然后再设置值 支持分布式，可以更细粒度的控制 多台机器上多个线程对一个数据进行操作的互斥。
	 * Redis是单线程的!!!
	 * 
	 * @param productId
	 * @throws Exception
