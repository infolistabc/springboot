...
public class LicenseGenerator {
    ...
    public static void generator() throws Exception {
        ...
        licenseBo.setEncryption(Base64Utils.encode(encodedData));

        //加密实体
        byte[] result = RSAUtils.encryptByPrivateKey(licenseBo.toString().getBytes(StandardCharsets.UTF_8),privateKey);

        System.out.println("result:"+Base64Utils.encode(result));

        Base64Utils.byteArrayToFile(Base64Utils.encode(encodedData).getBytes(StandardCharsets.UTF_8), FileUtil.getBasePath()+File.separator+"license.dat");
        //Base64Utils.byteArrayToFile(json.getBytes(StandardCharsets.UTF_8), FileUtil.getBasePath()+File.separator+"license.dat");
